...
for i in 1024 4096; do
    ...
    void* myThreadFun(void *vargp) {
        TEST_OP(memcpy, store_test, test2, test1, size, ACCESSES);
        TEST_OP(memcpy, ntstore_test, test2, test1, size, ACCESSES);
        return NULL;
    }
    ...
    void* myThreadFun2(void *vargp) {
        ...
        TEST_OP(memcpy_elide_clwb, store_test, &test2[tid * COPY_SIZE / sizeof(test2[0])], test1, size, ACCESSES);
        TEST_OP(memcpy_elide_clwb, ntstore_test, &test2[tid * COPY_SIZE / sizeof(test2[0])], test1, size, ACCESSES);
        return NULL;
    }
    ...
    {
        size = SIZE;
        test1 = (uint64_t*)aligned_alloc(PAGE_SIZE, size);
        test2 = (uint64_t*)aligned_alloc(PAGE_SIZE, size);
        printf(\"%p\n\", test1);
        printf(\"%p\n\", test2);
        printf(\"Copy size: %u\n\", COPY_SIZE);
